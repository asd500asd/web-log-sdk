...
class Heatmap {
  constructor(data) {
    ...
    data.forEach(item => {
      ...
      const { left, top, width, height, right, bottom } = getBoundingClientRect(elm);
      item.x = left + width / 2;
      item.y = top + height / 2;
      item.w = width / 2;
      item.h = height / 2;
  ...
  drawPointTemplate(point) {
    ...
    try {
      const gradient = ctx.createRadialGradient(point.x, point.y, point.w, point.x, point.y, point.h);
